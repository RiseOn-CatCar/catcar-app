...
public static class CompleteWorkOrderHandler
{
    public static async Task<Upshot<string>> Handle(CompleteWorkOrderCommand command, IValidator<CompleteWorkOrderCommand> validator, IWorkOrderRepository repository, IDbContextOutbox<ServiceOperationsDbContext> outbox, CancellationToken cancellationToken)
    {
        ...
        var transition = workOrder.Complete();
