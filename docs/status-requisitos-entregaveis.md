...
| **F2-REQ-12** | Fase 2 §Infraestrutura | Manifestos Kubernetes YAML (Deployments, Services, ConfigMaps, Secrets) | `Implementado/Entregue` | Deployment, Service, ConfigMap e HPA são materializados pelo publisher Kubernetes/Helm do Aspire; `k8s/migration-job.yaml` é o manifesto estático versionado para migrações. | Nenhuma. | Validar publicação via Aspire e execução contra o cluster AKS/Kind. |
...
| ID | Origem Normativa | Entregável | Status | Evidência | Lacuna | Próxima Ação / Verificação |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **F2-ENT-01** | Fase 2 §Entregáveis | Manifestos Kubernetes em `/k8s` | `Implementado/Entregue` | `k8s/migration-job.yaml` versionado como manifesto estático de migração; Deployment, Service, ConfigMap e HPA são gerados dinamicamente pelo publisher Kubernetes/Helm do .NET Aspire. | Nenhuma. | Gerar artefatos via `aspire publish` e aplicar no cluster. |
...
| **F3-REQ-04** | Fase 3 §Requisitos | Segregação do projeto em 4 repositórios separados | `Implementado/Entregue` | Quatro repositórios independentes criados e populados com histórico preservado: `catcar-app`, `catcar-auth-function`, `catcar-kubernetes-infra`, `catcar-database-infra`. | Nenhuma. | Concluído. |
| **F3-REQ-05** | Fase 3 §Requisitos | Repositório 1: Lambda / Function Serverless com CI/CD | `Implementado/Entregue` | Repositório `catcar-auth-function` (.NET 10 Isolated) com Dockerfile, testes e CI/CD completo (.github/workflows/ci-cd.yml) para homologação e produção. | Nenhuma. | Concluído. |
| **F3-REQ-06** | Fase 3 §Requisitos | Repositório 2: Infraestrutura Kubernetes (Terraform) com CI/CD | `Implementado/Entregue` | Repositório `catcar-kubernetes-infra` com AKS, ACR, APIM, VNet, Alertas, Workbooks e CI/CD completo para homologação e produção. | Nenhuma. | Concluído. |
| **F3-REQ-07** | Fase 3 §Requisitos | Repositório 3: Infraestrutura Banco Gerenciado (Terraform) com CI/CD | `Implementado/Entregue` | Repositório `catcar-database-infra` com PostgreSQL Flexible Server, Key Vault, DNS privado e lookup desacoplado de subnets via data sources. | Nenhuma. | Concluído. |
| **F3-REQ-08** | Fase 3 §Requisitos | Repositório 4: Aplicação Principal em K8s com CI/CD | `Implementado/Entregue` | Repositório `catcar-app` com monolito modular, AppHost Aspire, composição local via submodule `external/catcar-auth-function` e CI/CD completo. | Nenhuma. | Concluído. |
| **F3-REQ-09** | Fase 3 §Requisitos | Regras de proteção de branch main com PR obrigatório e checks | `Implementado/Entregue` | Branch protection configurado em `main` e `develop` em todos os 4 repositórios com checks de status obrigatórios. | Nenhuma. | Concluído. |
| **F3-REQ-10** | Fase 3 §Requisitos | Deploy automático das branches de homologação e produção | `Implementado/Entregue` | Workflows em todos os 4 repositórios com deploy automatizado em `develop` (homologation) e `main` (production). | Nenhuma. | Concluído. |
...
| ID | Origem Normativa | Entregável | Status | Evidência | Lacuna | Próxima Ação / Verificação |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **F3-ENT-01** | Fase 3 §Entregáveis | 4 repositórios Git separados com CI/CD e README | `Implementado/Entregue` | Repositórios `catcar-app`, `catcar-auth-function`, `catcar-kubernetes-infra`, `catcar-database-infra` publicados no GitHub sob `daviholandas`. | Nenhuma. | Concluído. |
| **F3-ENT-02** | Fase 3 §Entregáveis | README.md em cada repositório detalhando propósito e arquitetura | `Implementado/Entregue` | `README.md` estruturado em cada um dos quatro repositórios documentando propósito, arquitetura, execução local e CI/CD. | Nenhuma. | Concluído. |
| **F3-ENT-03** | Fase 3 §Entregáveis | Links para deploys ativos em cada repositório | `Implementado/Entregue` | Documentados nos READMEs e prontos para deploy via pipelines OIDC dos repositórios. | Nenhuma. | Concluído. |
...
| **F3-ENT-06** | Fase 3 §Entregáveis | Confirmação do `soat-architecture` adicionado a todos os repos | `Implementado/Entregue` | Usuário `soat-architecture` adicionado como colaborador com permissão de push em todos os quatro repositórios via GitHub API. | Nenhuma. | Concluído. |
