...
public class ListWorkOrdersHandlerTests
{
    ...
    public async Task Handle_WithoutFilters_ShouldReturnAllSummaries()
    {
        ...
        _repository.ListAsync(null, null, Arg.Any<CancellationToken>()).Returns(new List<WorkOrder> { workOrder });
    ...
    {
        _repository.ListAsync(Arg.Any<Guid?>(), WorkOrderStatus.Received, Arg.Any<CancellationToken>()).Returns(new List<WorkOrder>());

        await ListWorkOrdersHandler.Handle(new ListWorkOrdersQuery(null, "Received"), _repository, CancellationToken.None);

        await _repository.Received(1).ListAsync(Arg.Any<Guid?>(), WorkOrderStatus.Received, Arg.Any<CancellationToken>());
    }
    ...
    public async Task Handle_WithInvalidStatusFilter_ShouldFailWithoutHittingRepository()
    {
        ...
        await _repository.DidNotReceive().ListAsync(Arg.Any<Guid?>(), Arg.Any<WorkOrderStatus?>(), Arg.Any<CancellationToken>());
    }
    ...
    public async Task Handle_WithCustomerIdFilter_ShouldForwardToRepository()
    {
        ...
        _repository.ListAsync(customerId, null, Arg.Any<CancellationToken>()).Returns(new List<WorkOrder>());

        await ListWorkOrdersHandler.Handle(new ListWorkOrdersQuery(customerId, null), _repository, CancellationToken.None);

        await _repository.Received(1).ListAsync(customerId, null, Arg.Any<CancellationToken>());
    }
}
